...
優先度スコアは、以下の5つの要素の合計で計算されます（最大150点）：
...
記事のテーマに合致するキーワードを優先するためのスコアです。

- **完全一致**: 50点（記事タイトルにキーワードが完全に含まれている場合）
- **部分一致**: 0-40点（キーワード内の単語がタイトルに含まれている割合に基づいて計算）
  - 計算式: `(一致した単語数 / キーワードの単語数) × 40`
  - 例: キーワード「ペットロス ロボット」でタイトルに「ペットロス」「ロボット」が両方含まれている場合 → 40点
  - 例: キーワード「ペットロス ロボット」でタイトルに「ペットロス」のみ含まれている場合 → 20点

**理由**: 記事のテーマに合致するキーワードを優先することで、記事の内容がぶれず、より適切な改善案を提供できる。1位のキーワードでも、記事タイトルとの関連性が高い場合は選定され、2,3位の競合の情報を取得することで防御策として機能する。
...
| キーワード | インプレッション | クリック | 順位 | CTR | 関連性 | 通常スコア | 転落キーワードスコア |
|-----------|----------------|---------|------|-----|--------|-----------|-------------------|
| ポケとも 価格 | 1,000 | 50 | 10位 | 5% | 30点 | 50+15+10+5+30=110点 | **220点** |
| ポケとも 口コミ | 500 | 30 | 15位 | 6% | 40点 | 25+9+5+5+40=84点 | **168点** |

### 例2: 通常キーワード

| キーワード | インプレッション | クリック | 順位 | CTR | 関連性 | スコア |
|-----------|----------------|---------|------|-----|--------|--------|
| ポケとも レビュー | 800 | 40 | 8位 | 5% | 25点 | 40+12+10+5+25=92点 |
| ポケとも 評判 | 300 | 20 | 12位 | 6.7% | 20点 | 15+6+5+5+20=51点 |
